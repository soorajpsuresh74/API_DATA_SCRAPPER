...
logging.basicConfig(
    filename='EachOrganizationEndpoint.log',
    level=logging.INFO,
    ...
    if combined_data:
        enriched_data = [
            {**data, 'Title': row['title'],
             ...
             'Has Grow With SAP': row['hasGrowWithSap'],
             'Consultants': row['consultants'],
             'Description': row['description']}
            for data in combined_data
        ]
        all_combined_data.extend(enriched_data)
        success_count += 1
    else:
        failure_count += 1

combined_df = pd.DataFrame(all_combined_data)
combined_df.to_csv(r'files/combined_company_data.csv', index=False)
